...
class StudentProfile(BaseModel):
    ...
    user = models.OneToOneField(  settings.AUTH_USER_MODEL,
        ...
        related_name='student_profile',verbose_name=_("User Account")
    )

    major = models.ForeignKey( Major,
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name="students",verbose_name=_("Major")
    )
    ...
    cv_file = models.FileField( _("CV File"),
    ...
    def is_profile_complete(self):
        required_fields = [self.skills,self.graduation_year,self.gpa,self.major]
        return all(required_fields)

    @property
    def complete_profile_persent(self):
        required_fields = [
            self.first_name,self.last_name,self.phone_number,self.picture,
            self.graduation_year,self.skills,self.graduation_year,self.gpa,self.major
        ]
        availabels=0
    ...
    def get_major(self):
        if(self.major):
            return self.major.name or None
        return None
    @property
